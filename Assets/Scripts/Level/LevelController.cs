...
public class LevelController : MonoBehaviour, IService {
    ...
    private void NextLevel(NextLevelSignal signal) {
        int levelCount = _levelLoader.GetLevels().Count();
        if (_currentLevelId < levelCount - 1) {
            _currentLevelId++;
        }
    ...
    private void SelectLevel(int level) {
        ...
        _eventBus.Invoke(new ChangeBackgroundSignal(_currentLevelData.LevelBackgroundSprite));
    }
